...
public class Auth
{
    ...
    private void app_Initialize()
    {
        if (Started)
        {
            Dictionary<string, object> result = JsonConvert.DeserializeObject<Dictionary<string, object>>(AESCrypt.Decrypt(httpControl.Post(authHelpers.APIStartup + "?initialize", "data=" + AESCrypt.Encrypt(JsonConvert.SerializeObject(new Dictionary<string, string>
            {
            ...
            switch (GetSecurity.getMD5((string)result["status"]))
            {
                case "260CA9DD8A4577FC00B7BD5810298076": // success <3
                    ...
                    {
                        if (Auth.App_Version != App.Version)
                        {
    ...
    internal bool Login(string username, string password)
    {
        if (Started)
        {
            ...
            Dictionary<string, object> result = JsonConvert.DeserializeObject<Dictionary<string, object>>(AESCrypt.Decrypt(httpControl.Post(authHelpers.APIStartup + "?login", "data=" + AESCrypt.Encrypt(JsonConvert.SerializeObject(new Dictionary<string, string>
            {
                ["username"] = username,
                ["password"] = password,
    ...
    internal bool Register(string username, string password, string license)
    {
        if (Started)
        {
            Dictionary<string, object> result = JsonConvert.DeserializeObject<Dictionary<string, object>>(AESCrypt.Decrypt(httpControl.Post(authHelpers.APIStartup + "?register", "data=" + AESCrypt.Encrypt(JsonConvert.SerializeObject(new Dictionary<string, string>
            {
                ["username"] = username,
                ["password"] = password,
                ["license"] = license,
    ...
    internal bool ExtendTime(string username, string password, string license)
    {
        if (Started)
        {
            ...
            Dictionary<string, object> result = JsonConvert.DeserializeObject<Dictionary<string, object>>(AESCrypt.Decrypt(httpControl.Post(authHelpers.APIStartup + "?extend", "data=" + AESCrypt.Encrypt(JsonConvert.SerializeObject(new Dictionary<string, string>
            {
                ["username"] = username,
                ["password"] = password,
                ["license"] = license,
    ...
    internal bool licenseLogin(string license)
    {
        if (Started)
        {
            ...
            Dictionary<string, object> result = JsonConvert.DeserializeObject<Dictionary<string, object>>(AESCrypt.Decrypt(httpControl.Post(authHelpers.APIStartup + "?license", "data=" + AESCrypt.Encrypt(JsonConvert.SerializeObject(new Dictionary<string, string>
            {
                ["license"] = license,
                ["hwid"] = GetSecurity.HWIDPC(),
                ["ip"] = GetSecurity.IP(),
    ...
    internal string Variable_capture(string var_code)
    {
        ...
        else if (Started)
        {
            ...
            Dictionary<string, object> result = JsonConvert.DeserializeObject<Dictionary<string, object>>(AESCrypt.Decrypt(httpControl.Post(authHelpers.APIStartup + "?variable", "data=" + AESCrypt.Encrypt(JsonConvert.SerializeObject(new Dictionary<string, string>
            {
                ["action"] = "variable",
    ...
    internal bool sendLog(string message)
    {
        if (Started)
        {
            Dictionary<string, object> result = JsonConvert.DeserializeObject<Dictionary<string, object>>(AESCrypt.Decrypt(httpControl.Post(authHelpers.APIStartup + "?log", "data=" + AESCrypt.Encrypt(JsonConvert.SerializeObject(new Dictionary<string, string>
            {
                ["username"] = Info.Username,
                ["hwid"] = GetSecurity.HWIDPC(),
                ["message"] = message,
...
public class SecureConnections : Auth
{
    ...
    { 
        ...
        if (DateTime.Now.Ticks - DateTime.Now.Ticks < 10L)
            return true;        
    ...
    private static extern bool checkRemoteDebugger(IntPtr hProcess, ref bool debugger);
    private bool antiDebugger()
    {
        bool debugger = false;
        
        try
        {
            checkRemoteDebugger(System.Diagnostics.Process.GetCurrentProcess().Handle, ref debugger);
            return debugger;
        }
        catch { return debugger; }
    }
    #endregion
}
